'use client';
import { getOwnerProjects, getTrackingById, getNextGateByProjectProjectId } from "@/axios/ProjectApis";
...
export default function ProjectList() {
    ...
    useEffect(() => {
        if (!user.preferred_username) return; // Wait until username is available
        ...
        const fetchProjects = async () => {
            ...
            try {
                ...
                // Filter projects based on next gate availability
                const filteredProjects: Project[] = [];

                for (const project of projectsWithTracking) {
                    const response = await getNextGateByProjectProjectId(Number(project.id));
                    const data = response.data
                    console.log("hasNextGate: ", response);
