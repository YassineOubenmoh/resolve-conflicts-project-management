...
import SpocLayout from '@/app/spoc/SpocLayout';
...
const AddImpact = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Flush state changes before navigating
      setTimeout(() => {
        router.replace("/spoc/impact-documents/"); // use replace to avoid back button going to form
      }, 500); // optional delay for success message
        ...
        <ComponentCard className="w-full max-w-lg bg-white p-8 rounded-xl shadow-xl flex flex-col space-y-6">
          ...
          <div className="flex justify-center">
            <button
              ...
              disabled={loading}
              className={`text-white px-6 py-3 rounded-lg transition-all ease-in-out duration-200 ${
                loading ? 'bg-[#a02472]' : 'hover:bg-[#B12B89]'
              }`}
