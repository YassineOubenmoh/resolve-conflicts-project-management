...
import {
  getTrackingPercentagesByDates,
  getMarketTypePercentagesByDates,
  getProjectTypePercentagesByDates,
  getTtmProjectsByDates,
...
const DashboardSecondSpoc = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!department || !fromDate || !toDate) return;
      ...
      try {
        const [trackingRes, marketRes, projectRes, ttmRes] = await Promise.all([
          getTrackingPercentagesByDates(from, to, department),
          getMarketTypePercentagesByDates(from, to, department),
          getProjectTypePercentagesByDates(from, to, department),
          getTtmProjectsByDates(from, to, department),
        ]);

        setTrackingData(trackingRes.data);
        setMarketData(marketRes.data);
        setProjectData(projectRes.data);
        setTtmData(ttmRes.data);
      } catch (error) {
              ...
              <div className="flex flex-wrap gap-4 mb-4">
                {trackingData.map((entry, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    ...
                    ></div>
                    <span className="text-sm text-gray-700 font-medium">{entry.trackingType}</span>
                  </div>
                ))}
              </div>
        ...
        {ttmData.length > 0 && (
          <div className="bg-white rounded-2xl p-6 shadow-md w-full max-w-6xl mx-auto">
            ...
            <table className="w-full table-auto border-collapse">
              ...
              <tbody>
                {ttmData
                  .filter((item) => item.projectTitle && item.ttm !== undefined)
                  .map((project, index) => (
                    <tr
                      key={index}
...
export default DashboardSecondSpoc;
